...
Artificial neural networks, despite the name, are still very different from how brains work. Most networks are trained using top-down gradient-based methods that are not remotely biologically realistic. Training a state-of-the-art deep learning model such as GTP-3 or DALL-E2 can cost millions of dollars and requires specific infrastructure to deploy. Meanwhile, our brains generate general intelligence, trained unsupervised. The brain has an energy consumption of about 20 Watt, which is about three cans of coke a day. Brains are remarkably efficient.
...
Hyperdimensional computing exploits the power of high dimensions by smearing out the information over the complete vector. A distributed, holographic representation allows building a remarkable robust and efficient learning system. Because HDC can be done using efficient bit-operations, it can be made very energy-efficient. Furthermore, a basic HDC system is relatively easy to implement, as we will see. 
"""
...
To buian ld HDC system, we, of course, need the HDVs themselves. The HDVs are just ordinary vectors. Next, we need suited arithmetic operations to manipulate these vectors.
"""
...
md"We expect about have of the elements of two randomly-chosen vectors to match."
...
Our last operation to create new vectors is a simple unitary one that can create a new HDV from an old one. For example, when one wants to include positional information. An easy trick is to perform **shifting** using cyclic permutation. Just move every element of the HDV one or more steps to the right. Elements that fall off are placed back at the beginning. 
"""
...
Bundling, binding, and shifting allow for meaningful computation on the concepts your HDV represent. One vital property of binding and shifting possess is that they are *reversible: applying them does not remove any information. Binding also distributes over bundling:
"""
...
md"This means that transforming a bundle of concepts with binding is equivalent to binding every element before binding.
...
md"Now that we can encode emji and colors, we can represents couples with yet another binding operation. Subsequently, we can embed the complete dataset by bundling."
...
HDC seems to complement deep learning quite well. For 1D sequence-like data, HDC has been shown to outcompete convolutional neural networks. The latter still deliver superior performance for 2D image-like data at the expense of a much more considerable computation expense. I got intrigued by HDC by [a paper](https://arxiv.org/abs/2106.02894) that showed it works about as good as graph convolutional neural networks for predicting drug-like properties while taking mere minutes to train. HDC is an energy-efficient alternative to deep learning, with potential in fields such as [robotics](https://www.science.org/doi/10.1126/scirobotics.aaw6736).
